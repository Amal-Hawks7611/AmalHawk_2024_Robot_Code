...
public class RobotContainer {
  ...
  private void configureBindings() {
    ...
    if (Robot.isSimulation()) {
      ...
      driverPs5.povUp().onTrue(Commands.runOnce(() -> drivebase.resetOdometry(new Pose2d(3, 3, new Rotation2d()))));
    ...
    if (DriverStation.isTest()) {
      ...
      driverPs5.cross().whileTrue(Commands.runOnce(drivebase::lock, drivebase).repeatedly());
      driverPs5.triangle().whileTrue(drivebase.driveToDistanceCommand(1.0, 0.2));
      driverPs5.povUp().onTrue((Commands.runOnce(drivebase::zeroGyro)));
      driverPs5.povDown().whileTrue(drivebase.centerModulesCommand());
    } else {
      driverPs5.povUp().onTrue((Commands.runOnce(drivebase::zeroGyro)));
    }
